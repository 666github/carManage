$(document).ready(function() {
    ...
    getDepartment(".illInputtingBranch");
    getDepartment(".breakInputtingBranch");
	checkMessage();
//	getAllMessage();	
	getAllMessage1();
});
	...
	function getDepartment(department){
		...
		$.ajax({
			...
			success: function (data) {
				...
				$(department).val(currentBranch);
				getPeopleName(".illInputtingBranch",".illInputtingPerson");debugger
  				getPeopleName(".breakInputtingBranch",".breakInputtingPerson");debugger
			},
			error: function () {
	  			console.log("部门获取失败");
	  		}
    ...
    //人名 
   	function getPeopleName(branchClass,className){
   		var department=$(branchClass).val();debugger
		var option = $(className).html(" ");
		$.ajax({
	  		type: "post",
			dataType: "json",
			url: "http://192.168.1.120:2238/handler/GetAllDrivers.ashx",
   	...
   	//部门改变获取本部门司机
   	$("#illInputtingBranch").change(function(){
   		getPeopleName(".illInputtingBranch",".illInputtingPerson");
   	});
   	$("#breakInputtingBranch").change(function(){
   		getPeopleName(".breakInputtingBranch",".breakInputtingPerson");
   	})
//切换消息模块
...
function getAllMessage1(){
	...
	$.ajax({
		type:"get",
		url:"http://192.168.1.120:2238/handler/Index/IndexMessage.ashx?info=1",
//		url:"http://127.0.0.1:8020/CarManageSystem/CarManageSystem/data/首页.json",
	...
	this.getAudit = function (_data,_data1,_data2,_data3) {
		...
		for (var i=0; i<_data3.length;i++) {//myauditCar 我跨院借车审批//夜间周末用车
		    auditNode += '<div class="auditNode">' +
				...
				'<div class="auditIcon">' +
					'<div>提交于<span class="applyTime">' + _data3[i].applyTime + '</span><span class="handleNode" style="margin-left:10px">' + _data3[i].handleNode + '</span></div>' +
				'</div>' +
			'</div>';
		}
...
//			var arry = [name];
			...
			$.ajax({
				type:"get",
				url: "http://192.168.1.120:2238/handler/CarManage/AccessBorrow.ashx",
...
function getAllMessage2(){
	...
	$.ajax({
		type:"get",
		url:"http://192.168.1.120:2238/handler/Index/IndexMessage.ashx?info=2",
		xhrFields:{withCredentials:true},
		data:{from:0},
		dataType:"json",
		success:function(data){
			if(data.state=="success"){
			    content.getRemind(data.remindIlleCar,data.peccancy,data.remindOut,data.myremindIlleCar,data.myremindOut,data.pushinfo);
			}else if(data.state=="1"){
				alert("请重新登录");
				window.location.href="./login.html"
			}else{
	...
	this.getRemind = function (_data1,_data2,_data3,_data4,_data5,_data6) {
	    getIllage(_data1,_data2,_data3,_data4,_data5,_data6);
	...
	function getIllage(_data1,_data2,_data3,_data4,_data5,_data6){
		...
		for (var i=0;i<_data1.length;i++){
			illegalNode += '<div class="illegalNode">'+
				'<div class="driverPic">'+
					'<p class="remindTitle">违规用车</p>'+
				'</div>'+
				'<div class="illegalPeople hide">'+
					'<span>违规用车人：</span><span class="illperson"></span>'+
					'<span>违规用车原因：</span><span class="illreason"></span>'+											
				'</div>'+
				'<div class="carInfo">'+
				...
				'<div class="breakRuleInfo">' +
					'<div class="breakRuleNode">'+
						'<div class="carInfo">'+
							...
							'<span title='+_data2[i].reason+'>违章原因:'+_data2[i].reason+'</span>'+
...
//				'</div>'+
		...
		//非工作日用车
		for(var i = 0; i<_data6.length; i++){
		    illegalNode += '<div class="illegalNode">'+
				'<div class="driverPic">'+
					'<p class="remindTitle">非工作日用车</p>'+
				'</div>'+
				'<div class="noworkdaysTitle">'+
					'<span class="" style="width:25%;display:inline-block;">'+_data6[i].personName+'</span>'+
					'<span class="brand">车牌：'+_data6[i].carLicence+'</span>'+
				'</div>' +
				'<div class="carInfo">'+
					'<span class="nowrokdaysTime">时间：'+_data6[i].time+'</span>'+
					'<span class="carLicence">类型：'+_data6[i].type+'</span>'+
				'</div>'+
			'</div>';
		}
	...
	this.disposes = function(){
		...
		$(".quitSure #sureIll").unbind("click").click(function(){	
			...
			if(illReason!=""){
				$.ajax({
				 	type:"get",
					url:"http://192.168.1.120:2238/handler/CarManage/HandleIllegalUseCar.ashx",
					xhrFields: {withCredentials: true }, 
					...
					success:function(data){
						if(data.state=="success"){
							othis.addClass("hide");
							othis.parents(".illegalNode").find(".transportTime").append("<span class='illperson'>"+(PeopleName==null?"":PeopleName)+"</span>违规用车原因：</span><span class='illreason'>"+illReason+"</span>");
							$(".illInputting").addClass("ohide");
							getAllMessage2();
						}else if(data.state=="1"){
							alert("请重新登录");
		...
		$(".quitSure #sureBreak").unbind("click").click(function(){	
			...
			$.ajax({
				type:"get",
				url:"http://192.168.1.120:2238/handler/CarManage/Illegal/HandleIllegal.ashx",
				xhrFields: {withCredentials: true }, 
	...
	//违法补录信息
		...
		$(".illegalSave").bind("click",function(){
			...
			$.ajax({
				type:"get",
				url:"http://192.168.1.120:2238/handler/CarManage/AddInfo.ashx",
				xhrFields: {withCredentials: true }, 
...
function getAllMessage3(){
	...
	$.ajax({
		type:"get",
		url:"http://192.168.1.120:2238/handler/Index/IndexMessage.ashx?info=3",
//		url:"http://127.0.0.1:8020/CarManageSystem/CarManageSystem/data/首页.json",
		xhrFields:{withCredentials:true},
		data:{from:0},
		dataType:"json",
...
function getAllMessage4(){
	...
	$.ajax({
		type:"get",
		url:"http://192.168.1.120:2238/handler/Index/IndexMessage.ashx?info=4",
//		url:"http://127.0.0.1:8020/CarManageSystem/CarManageSystem/data/首页.json",
		xhrFields:{withCredentials:true},
		data:{from:0},
		dataType:"json",
