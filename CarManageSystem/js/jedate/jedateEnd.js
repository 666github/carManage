...
;(function(root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        root.jeDateEnd = factory(window.jQuery || $);
    }
})(this, function($) {
    ...
    $.fn.jeDateEnd = function(options){
        return this.each(function(){
            return new jeDateEnd($(this),options||{});
        });
    };
    $.extend({
        jeDateEnd:function(elem, options){
    ...
    function jeDateEnd(elem, opts){
        this.opts = opts;
        this.valCell = elem;
        this.init();
    }
    var jedfn = jeDateEnd.prototype, jefix = "jefixed";
    jedfn.init = function(){
...
//-------------------
                    if(!($(".jedateprophours .action").prev().hasClass("disabled"))){
                    	$(".jedatepropminutes .action").prevAll().removeClass("disabled")
                    }else{
                    	$(".jedatepropminutes .action").prevAll().addClass("disabled")
                    }
                    if(hmsarr.length != 0 && (hmsarr[i] != undefined && hmsarr[i] != "zz")) {
