    ...
    function carsInf(car_inf,arr_carImg,arr_brand1,arr_licence,arr_space,arr_state,arr_oilMass,arr_site,arr_lastperson){
			for(var i=0;i<car_inf.length;i++){
				if(arr_state[i]==0){
					...
					arr_state[i]="限号";
				}else if(arr_state[i]==10){
					arr_state[i]="非法(已处理)";
				}else if(arr_state[i]==11){
					arr_state[i]="被预约(待审)";
				}else{
					arr_state[i]="已预约";
			...
			$(".carState").each(function(){
				...
				if($(this).text().indexOf("空")==0 || $(this).text().indexOf("申")==0|| $(this).text().indexOf("被")==0|| $(this).text().indexOf("节")==0){
					$(this).addClass("nowOrder");
...
var carLicenceWX="";
	...
	$(".quitSure #sureAdm").unbind("click").click(function(){		
			...
			if(resontextAdm!=""){
				$.ajax({
					url:"http://192.168.1.120:2238/handler/CarManage/CarQuery/DirectMaintain.ashx",
					xhrFields: {withCredentials: true }, 
...
//删除信息
	$(".odel").click(function(){
		...
		if(confirm("确定删除？")){
			$.ajax({
				url:"http://192.168.1.120:2238/handler/CarManage/DeleteCar.ashx",
				xhrFields: {withCredentials: true}, 
...
////预约车辆弹框
	function order(){
		...
		node.find(".nowOrder").bind("click",function(){
			if($(this).text()=="被预约(待审)"){
				if(!confirm("此车已有人预约，正在等待审核，您是否确定继续借车")){
					return;
				}
			}
