	...
	function getPersonName(){debugger
		...
		var currentBranch = $(".allBranch").val();
         $.ajax({
            type: "post",
            dataType: "json",
            url: "http://192.168.1.120:2238/handler/GetDrivers.ashx",
            data:{"department":currentBranch,from:0},
            xhrFields:{withCredentials:true},
            success: function (data) {
                var obj = data.users;
...
//查询	
	...
	$.ajax({
		type:"get",
		url:"http://192.168.1.120:2238/handler/Statistics/QueryDriverTrajectory.ashx",
		async:false,
		data:{"department":department,"personnelName":personnelName,"carNumber":carNumber,"usedTime1":usedTime1,"usedTime2":usedTime2,from:0},
		xhrFields:{withCredentials:true},
		success:function(data){
			...
			if(data.state=="success"){
				...
				for(var i=0;i<usedInfs.length;i++){
					...
					'<li class="list5 list52" title="'+usedInfs[i].list5+'">'+usedInfs[i].list5+'</li><li class="list6 list62" title="'+usedInfs[i].list6+'">'+(usedInfs[i].list6==""?"无":usedInfs[i].list6)+'</li>'+
					'<li class="list7">'+usedInfs[i].list7+'</li><li class="list8">'+usedInfs[i].list8+'</li>'+
					'<li class="list11">'+usedInfs[i].list11+'</li><li class="list10">'+usedInfs[i].list10+'</li>'+
...
//导出
	function exportInf(){
        ...
        location.href = 'http://192.168.1.120:2238/handler/Statistics/ExportExcelForDriverTrajectory.ashx?from=0&id=' + exportId;
		//$.ajax({
  //          type: "post",
  //          dataType: "json",
