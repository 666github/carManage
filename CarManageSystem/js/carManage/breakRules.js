	  				...
	  				if(userType==1){
	  					$.each($(".mapfirList p"), function() {
	  						if($(this).text()!=$(".branchName").text()){
	  							...
	  							$(".illLists").css("height","calc(100% - 70px)")
	  						}
	  					});
	  				}
	  				listclick();
	  				$(".mapsecList").on("click","li",function(carLicence){
	  					$(".mapsecList li").css("color","#000000");
	  					$(this).css("color","red");
	  					carLicence=$(this).text();
						queryBrearule(carLicence);
					})
...
//---	
	function queryBrearule(carLicence){debugger
		var breakRuleInf="";		
		$.ajax({
			type:"get",
