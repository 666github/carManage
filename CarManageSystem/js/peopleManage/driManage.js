...
//	//请求司机信息
	...
	function driverTask(){	
		...
		$.ajax({
			...
			data:{"driverType":driverType,"viewType":0,from:0},
			dataType:"json",
			async:true,
	...
	function addNode(_data){debugger
		var driver = '';
...
//			}
			...
			driver += '<ul class="driverInfo">'+
				...
				'</li>'+
				'<li class="driverState"><span class="free" datatype="driverState">'+_data[i].driverState+'</span><span class="details"  data="1">详情>></span></li>'+
			'</ul>';
		};
...
//		driApprove1();
			...
			for(var i=0;i<_data.length;i++){				
				...
				driver += '<ul class="driverInfo">'+
					...
					'<li class="driveingInfo">'+
						...
						'<p class="mailNum">邮箱 : <span class="mailNumInfo" datatype="mailNumInfo">'+_data[i].mailNumInfo+'</span></p>'+
					'</li>'+
					'<li class="driDate" style="text-align:center;padding-left:0"><span class="details detailshover" data="2">详情>></span><span class="effectDate" style="display:none;">'+_data[i].effectDate+'</span></li>'+
				'</ul>';
			};
	...
	if(page2==2){$(".driApprove").click();$("#driInfapprove").css("z-index",1)}//第二个详情返回时跳到审批页面
//同意与拒绝
	$(".oYes").click(function(){
