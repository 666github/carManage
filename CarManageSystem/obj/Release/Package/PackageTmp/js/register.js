...
//验证注册信息正确性
	function register() {
	    ...
	    var CarType = $("#regCarType").text();	   
	    if(regMyname.test(CarType)){
	    	alert("请选择准驾类型");
	    	return false;
	    }
	    var pwd = $("#regPwd").val();
	    ...
	    if($(".img1").attr('src')=='../img/sc.jpg' || $(".img2").attr('src')=='../img/sc.jpg'){
	    	alert("请上传照片");
	    	return false;
	    }
	    return true;
	}
	$(".zjcx span").click(function(event){
	...
	$(".zjcxInput input").click(function(){
		...
		if(arrchecked.length==0){
			...
			$(".zjcx span").text(arrchecked.join(","));
		}		
	});
	$("body").click(function(){
