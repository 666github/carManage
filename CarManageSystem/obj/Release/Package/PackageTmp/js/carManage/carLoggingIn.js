$(function(){
	//部门默认颜色设置
	(function(){
		$("select").css("color","darkgray");
		$("option").css("color","black")
		$("select").change(function(){
			var selItem=$(this).val();
			if(selItem==$(this).find("option:first").val()){
				$(this).css("color","darkgray")
			}else{
				$(this).css("color","black")
			}
		})
	})();
	//保养类别筛选
	$(".mtMileOrdate").change(function(){
		$("#maintainStart,#maintainEnd,#mtMile").toggle();
		$("#maintainStart,#maintainEnd,#mtMile").toggleClass("carLength");
		$("#maintainStart,#maintainEnd,#mtMile").val("");
	})
	//车辆录入信息验证
	function loggingIn(){
		var img1src=$(".img1").attr('src');
		if(img1src==""||img1src=="img/index/carbg.jpg"){
			alert('请上传车辆照片');
			return false;
		}
	    $.each($(".carLength"),function(){
	    	if($(this).val().length==0){
	    		alert("所有选项必填，有未填写选项！");
	       		return false;
	    	}
	    })
	    var regPlate=/^京[A-Z]{1}[A-Z0-9]{5}$/;//var regPlate=/^[\u4e00-\u9fa5]{1}[A-Z][A-Z0-9]{5}/
	    var oPlate=$("#regPlate").val().toUpperCase();
	    if (!regPlate.test(oPlate)) {
	        alert("车牌格式不正确");
	        return false;
	    }
	    var regVIN=/^[A-Z0-9]{17}$/;//var regPlate=/^[\u4e00-\u9fa5]{1}[A-Z][A-Z0-9]{5}/
	    var oVIN=$("#otype").val().toUpperCase();
	    if (!regVIN.test(oVIN)) {
	        alert("车架号为17位字母数字组合");
	        return false;
	    }
	    var regModel=/^[0-9]+$/;
	    var oModel=$("#regModel").val();
	    if (!regModel.test(oModel)) {
	        alert("车载人数为纯数字");
	        return false;
	    }	    
	    var regMileage=/^[0-9]*\.?[0-9]*$/;
	    var oMileage = $("#regMileage").val();
	    var mtMile = $("#mtMile").val();
	    if (!regMileage.test(oMileage) || !regMileage.test(mtMile)) {
	        alert("里程格式为纯数字或数字加小数点");
	        return false;
	    }
	    var regPrice=/^[0-9]*\.?[0-9]*$/;
	    var oPrice = $("#regPrice").val();
	    if (!regPrice.test(oPrice)) {
	        alert("价格格式为纯数字或数字加小数点");
	        return false;
	    }
	    var regEngin=/^[a-zA-Z0-9]{4,}$/
	    var oEngin = $("#regEngin").val();
	    if (oEngin.length < 4) {
	        alert("发动机号不能少于4位！");
	        return false;
	    }
	    if (!regEngin.test(oEngin)) {
	        alert("发动机号只能由字母、数字组成！");
	        return false;
	    }
	    //试用暂时注销
//	    var regICCID=/^[0-9]{20}$/;
//	    var oICCID=$(".ICCID").val();
//	    if(!regICCID.test(oICCID)){
//	    	alert("请输入20位纯数字！");
//	    	return false;
//	    }
	    return true;
	    
	}
	
	$("#mysubmit").click(function(){
		if(loggingIn()){
			$("#myform").submit();
		}
	})
})
