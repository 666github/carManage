    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.state=="success") {
                ...
                $(".mileDet").val(carInf_det.mileDet);
                $(".outnumDet").text(carInf_det.outnumDet);
                $(".mtStartDet").val(carInf_det.mtStartDet);
                $(".mtEndDet").val(carInf_det.mtEndDet);
                $("#mtMile").val(carInf_det.mtMile == "" ? 0 : carInf_det.mtMile);
                $(".mtMileOrdate").val(carInf_det.mtMileOrdate);
                if($(".mtMileOrdate").val()=="按里程保养"){
                	$("#mtStartDet,#mtEndDet,#mtMile").toggle();
					$(".maintan_det span").toggle();
					$("#mtStartDet,#mtEndDet,#mtMile").toggleClass("isnull");
                }
...
//          console.log($(".isnull")[i].value);
	    ...
	    var mtMile = $("#mtMile").val();
	    if (!regMileage.test(oMileage) || !regMileage.test(mtMile)) {
	        alert("里程为纯数字或数字加小数点");
	        return false;
	    }
...
//	        var oimg=$(".img1").attr("src");
			...
			$.ajaxFileUpload({
		        ...
		        dataType: 'json',
		        url: "../handler/CarManage/CarQuery/ChangeCarInfo.ashx",
		        xhrFields: { withCredentials: true },
		        data:{	       	
		            ...
		            "enginDet": enginDet, "priceDet": priceDet, "yearStart": yearStart, "yearEnd": yearEnd,"mileDet":mileDet,"outnumDet":outnumDet, "mtStartDet": mtStartDet, "mtEndDet": mtEndDet,
		            "mtMile":mtMile,"mtMileOrdate":mtMileOrdate,"mtMoneyDet":mtMoneyDet,"mtNumDet":mtNumDet,"simId":simIdnum,from:0
		        },
