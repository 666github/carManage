...
//	gasCardList();
    ...
    function gasCardList () {
        ...
        var infLists = '';
        $.ajax({
            url: "../handler/FuelingCard/ViewCard.ashx",
            xhrFields: { withCredentials: true },
...
//		});
		...
		if(regCard()){debugger
			$.ajax({
				url:"../handler/FuelingCard/AddNewCard.ashx",
				xhrFields: {withCredentials: true }, 
	...
	function getCardNumF(){debugger;
		var option = $('.cardListF').html("");
         $.ajax({
            type: "post",
            dataType: "json",
            url: "../handler/FuelingCard/GetCardList.ashx",
            data:{"department":department,from:0},
            xhrFields:{withCredentials:true},           
            success:function(data){
				if(data.state=="success"){
...
//		var secondCardId=othis.parents(".gasQuery").find(".gasNums").text();	
		...
		$.ajax({
			type:"get",
			url:"../handler/FuelingCard/ViewDetail.ashx",
			async:false,
			xhrFields: {withCredentials: true}, 
...
//	})
    $("#gasInf").change(function () {
        gasInfChange();debugger
    })
})	
var gasInfChange = function () {debugger
    ...
    $.ajaxFileUpload({
        ...
        dataType: 'json',
        url: "../handler/FuelingCard/InputTable.ashx",
        xhrFields: { withCredentials: true },
        data: { from: 0 },
