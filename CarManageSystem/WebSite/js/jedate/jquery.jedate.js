...
;(function(root, factory) {
    ...
    jedfn.init = function(){
        ...
        var formatDate = function (cls,boxcell) {
            ...
            dateDiv.attr("author","zlj").css({"z-index": boxcell != "#jedatebox" ? "" : zIndex ,"position":reabsfix,"display":"block"});
    ...
    jedfn.createDaysHtml = function(ys, ms, opts, boxCell){
            ...
            if(dateOfLimit(year, month, b, true)){
                if(ends.length > 0){
                    if (jet.ymdDate == thisDate){
                        bCls = jet.ymdDate == thisDate ? "action" : (currOne != thisOne && thisOne == thisDate ? "action" : "disabled");
                    }else {
                        if(dayreg.test(b)){
                            bCls = ends[1] == true ? "disabled" : "disabled";
                        }else{
                            bCls = ends[1] == true ? "disabled":"disabled";
                        }
                    }
        ...
        for(var n = 1, nlen = 42 - count; n <= nlen; n++){
            ...
            if(ends.length > 0){
                ...
                nCls = dateOfLimit(nextM.y, nextM.m, n, false) ? "disabled" : "disabled";
            }
            ...
            var sethmsStrHtml = function () {
                ...
                $.each([ "hours", "minutes", "seconds" ], function(i, hms) {
                    var hmsCls = boxCell.find(".jedateprop" + hms),
                        ...
                        onhmsPCls = hmsCls.find("p");hmsCls.nextAll().addClass("disabled");
//------------------------------------------------限制时间 俩小时的都不可选择
           			$(".jedatepropminutes .action").addClass("ooo")
					if($(".jedateprophours .action").prev().hasClass("disabled")){						
						$(".jedateprophours .action").next().next().nextAll().addClass("disabled")
					}else if($(".jedateprophours .action").prev().prev().hasClass("disabled")){
						$(".jedateprophours .action").next().nextAll().addClass("disabled");
						$(".jedatepropminutes .ooo").prevAll().removeClass("disabled");
					}else{
						$(".jedateprophours .action").nextAll().addClass("disabled")
					}
                    if($(".jedateprophours .action").next().hasClass("disabled")){
                    	$(".jedatepropminutes .action").nextAll().addClass("disabled")
                    	$(".jedatepropminutes .ooo").prevAll().removeClass("disabled");
                    }else{
                    	$(".jedatepropminutes .action").nextAll().removeClass("disabled")
                    }
                    if(hmsarr.length != 0 && (hmsarr[i] != undefined && hmsarr[i] != "zz")) {
...
//$(function(){
//	$(".jedatehmscon .action").prevAll().addClass("disabled")
//console.log($(".action").prevAll().length)
//})