	...
	this.getDrivers = function(_data){
		...
		for (var i = 0; i<_data.length;i++) {
			driversNode += '<ul class="driverInfo">'+
                ...
                '<span class="ffycCount">非法用车次数 : <span class="ffycCountInfo">' + _data[i].illeaglUCNumInfo +'</span></span></li>'+
...
//信息导出
	$(".exportInf").click(function(){
		...
		var driTime1=$("#driTime1").val();
		var driTime2=$("#driTime2").val();
		location.href="../handler/Statistics/ViewDriverStatistics.ashx?from=0&department="+department+"&startDate="+driTime1+"&endDate="+driTime2+"&export=1";
	})
function odelSearch(){
