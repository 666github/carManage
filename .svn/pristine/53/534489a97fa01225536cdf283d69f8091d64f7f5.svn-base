...
;(function(root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        root.jeDateGas = factory(window.jQuery || $);
    }
})(this, function($) {
    ...
    $.fn.jeDateGas = function(options){
        return this.each(function(){
            return new jeDateGas($(this),options||{});
        });
    };
    $.extend({
        jeDateGas:function(elem, options){
    ...
    function jeDateGas(elem, opts){
        this.opts = opts;
        this.valCell = elem;
        this.init();
    }
    var jedfn = jeDateGas.prototype, jefix = "jefixed";
    jedfn.init = function(){
            ...
            var sethmsStrHtml = function () {
                ...
                $.each([ "hours", "minutes", "seconds" ], function(i, hms) {
                    var hmsCls = boxCell.find(".jedateprop" + hms),
                        ...
                        //------
                        $(".jedatepropminutes .action").addClass("oM");
                        $(".jedatepropseconds .action").addClass("oS");
                        if(($(".jedateprophours .action").next().hasClass("disabled"))){
	                    	$(".jedatepropminutes .oM").nextAll().addClass("disabled");
	                    	$(".jedatepropminutes .oM").prevAll().removeClass("disabled"); 
	                    }else{
	                    	$(".jedatepropminutes .action").siblings().removeClass("disabled");
	                    	$(".jedatepropseconds .action").siblings().removeClass("disabled");
	                    }
                    if(hmsarr.length != 0 && (hmsarr[i] != undefined && hmsarr[i] != "zz")) {
