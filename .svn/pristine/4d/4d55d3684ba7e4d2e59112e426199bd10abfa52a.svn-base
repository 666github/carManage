...
function returnCar(){
	...
	node.find(".stateBack").click(function(){debugger
	    ...
	    $("#retruntime").val(now);
	    referenceMile();
		function referenceMile(){
		    	$.ajax({
					type:"get",
					url:"http://192.168.1.120:2238/handler/PersonalCenter/Reservation/GetDistance.ashx",
					xhrFields: {withCredentials:true}, 
					dataType:'json',
					async:true,
					data:{
					...
					success: function (data) {debugger
						if(data.state=="success"){
							$(".referenceMile").text(data.distance);								
						}else if(data.state=="1"){
								alert("请重新登录");
							location.href="./login.html"
						}else{
...
//			}
			...
			$("#returnSure").unbind("click").click(function(){
			    debugger
			    var referenceMile=$(".referenceMile").text();
...
//				$("#returnSure").unbind("click");
			    ...
			    if(referenceMile-carMile>100 || carMile-referenceMile>100 ){
			    	if(confirm("填写里程与参考里程对比误差较大，是否确认还车")){returnInf();}			    	
			    }else{
			    	returnInf();
			    }
				function returnInf(){
