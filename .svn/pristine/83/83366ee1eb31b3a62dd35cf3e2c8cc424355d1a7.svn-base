	...
	this.getDrivers = function(_data){
		...
		for (var i = 0; i<_data.length;i++) {
			driversNode += '<ul class="driverInfo">'+
                ...
                '<span class="ffycCount">违规用车次数 : <span class="ffycCountInfo">' + _data[i].illeaglUCNumInfo +'</span></span></li>'+
...
//信息导出
	$(".exportInf").click(function(){
		var department=$(".driBranch").val();
		var driTime1=$("#driTime1").val();
		var driTime2=$("#driTime2").val();
		location.href="http://192.168.1.120:2238/handler/Statistics/ViewDriverStatistics.ashx?from=0&department="+department+"&startDate="+driTime1+"&endDate="+driTime2+"&export=1";
	})
function odelSearch(){
	$(".driBranch").change(function(){
		$("div.driversInfo").html('<img src="./img/loading2.gif" style="position: absolute;position: absolute;top: 50%;left: 50%;margin-top: -50px;margin-left: -50px;width: 100px;height: 100px;">');
		checkInfo();
	})
