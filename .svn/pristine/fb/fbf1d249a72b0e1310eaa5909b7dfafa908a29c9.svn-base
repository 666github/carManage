...
namespace JT808
{
    ...
    [AttributeUsage(AttributeTargets.Property)]
    public class CBDAutoHandler : ReadWriteHandlerAttribute
    {
        public override object Read(IProtocolBuffer buffer)
        {
            return buffer.ReadBCD(buffer.Length);
        }
    ...
    [AttributeUsage(AttributeTargets.Property)]
    public class ParamListHandler : ReadWriteHandlerAttribute
    {
        public ParamListHandler(Type type)
        {
            mType = type;
        }
        private Type mType;
        ...
        public override object Read(IProtocolBuffer buffer)
        {
            var list = new List<ParamObject>();
            while (buffer.Postion!=buffer.Length-1)
            {
                var a = buffer.ReadUInt();
                var b = buffer.Read();
                ulong c = 0;
                if (b == 4)
                    c = (ulong)buffer.ReadUInt();
                else if (b == 8)
                    c = buffer.ReadUInt64();
                else if (b == 0)
                    c = 0;

                list.Add(new ParamObject() { ID = a, Length = b, Value = c });
            }
            
            //byte[] data = buffer.Read(buffer.Length);
            IBitCustomType result = (IBitCustomType)Activator.CreateInstance(mType);
            result.Load(list);
            return result;
        }
