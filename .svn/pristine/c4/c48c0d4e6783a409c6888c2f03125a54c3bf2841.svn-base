...
function getAllMessage1(){
	...
	$.ajax({
		type:"get",
		url:"http://192.168.1.120:2238/handler/Index/IndexMessage.ashx?info=1",///handler/Index/MessageRemind.ashx
		...
		success: function (data) {
			if(data.state=="success"){
				...
				debugger;
				content.getAudit(data.audit, data.auditUser,data.pushinfo);
...
//				content.getBreakRule(data.breakRule);
	...
	this.getAudit = function (_data,_data1,_data2) {
	    var auditNode = "";
		...
		for (var i = 0; i < _data.length; i++) {
				...
				'<div class="auditIcon">' +
					'<div>提交于<span class="applyTime">' + _data[i].applyTime + '</span><span class="handleNode" style="margin-left:10px">' + _data[i].handleNode + '</span></div>' +
				'</div>' +
			'</div>';
		}
		//非工作日用车
		for(var i = 0; i<_data2.length; i++){
		    auditNode += '<div class="auditNode">'+
				'<div class="auditMessage" style="height: 50px; float: left;line-height: 45px;text-align: center;font-size: 16px; color: #4275f8;">'+
					'<p class="remindTitle">非工作日用车</p>'+
				'</div>'+
				'<div class="noworkdaysTitle">'+
					'<span class="" >'+_data2[i].personName+'</span>'+
					'<span class="noworkdaysbrand">车牌：'+_data2[i].carLicence+'</span>'+
				'</div>' +
				'<div class="noworkdayscarInfo">'+
					'<span class="nowrokdaysTime">时间：'+_data2[i].time+'</span>'+
					'<span class="nowrokdaystype">类型：'+_data2[i].type+'</span>'+
				'</div>'+
			'</div>';
		}
//		$(".auditNode").remove();
...
function getAllMessage2(){
	...
	$.ajax({
		type:"get",
		url:"http://192.168.1.120:2238/handler/Index/IndexMessage.ashx?info=2",///handler/Index/MessageRemind.ashx
		xhrFields:{withCredentials:true},
		data:{'from':0},
		dataType:"json",
		success: function (data) {
...
function getAllMessage3(){
	...
	$.ajax({
		type:"get",
		url:"http://192.168.1.120:2238/handler/Index/IndexMessage.ashx?info=3",///handler/Index/MessageRemind.ashx
		xhrFields:{withCredentials:true},
		data:{'from':0},
		dataType:"json",
		success: function (data) {
...
function getAllMessage4(){
	...
	$.ajax({
		type:"get",
		url:"http://192.168.1.120:2238/handler/Index/IndexMessage.ashx?info=4",///handler/Index/MessageRemind.ashx
		xhrFields:{withCredentials:true},
		data:{'from':0},
		dataType:"json",
		success: function (data) {
